...
if(bufferCount <= 30):
    if(minX > filteredValues[0]):
        minX = filteredValues[0]
    if(maxX < filteredValues[0]):
        maxX = filteredValues[0]
    if(minY > filteredValues[1]):
        minY = filteredValues[1]
    if(maxY < filteredValues[1]):
        maxY = filteredValues[1]
    if(minZ > filteredValues[2]):
        minZ = filteredValues[2]
    if(maxZ < filteredValues[2]):
        maxZ = filteredValues[2]
...
enoughBuffered = True
    ...
    if((maxX > maxY)  &  (maxX > maxZ)):
        ...
        prevVal = filteredValues[0]
        x = True
        y = z = False
elif (maxY > maxZ):
    threshold = (minY+maxY)/2
        prevVal = filteredValues[1]
        y = True
        x = z = False
    else:
        threshold = (minZ+maxZ)/2
        prevVal = filteredValues[2]
        z = True
        y = x = False
...
if(x):
    ...
    value = filteredValues[1]
else:
value = filteredValues[2]


if((value < threshold) & (prevVal > threshold)):
print(value)
    print(threshold)
        print(prevVal)
